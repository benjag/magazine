...
if( $wp_query->current_post == 0 && !is_paged() && is_front_page() ) { // Custom template for the first post on the front page
    if (has_post_thumbnail()) {
        ...
        echo '<a href="' . get_permalink() . '" title="' . __('Read ', 'pacificmagazine') . get_the_title() . '" rel="bookmark">';
...
if( $wp_query->current_post == 0 && !is_paged() && is_front_page() ) { 
    ...
    the_content( __( '', 'pacificmagazine' ) );
    echo '</div>';
    echo '<footer class="entry-footer continue-reading">';
    echo '<a href="' . get_permalink() . '" title="' . __('Read ', 'pacificmagazine') . get_the_title() . '" rel="bookmark">Read the article<i class="fa fa-arrow-circle-o-right"></i></a>'; 
    ...
    <?php echo '<a href="' . get_permalink() . '" title="' . __('Continue Reading ', 'pacificmagazine') . get_the_title() . '" rel="bookmark">Continue Reading<i class="fa fa-arrow-circle-o-right"></i></a>'; ?>
    </footer><!-- .entry-footer -->
<?php } ?>
	</div><!-- .index-box -->
</article><!-- #post-## -->